...
| Seccion | Que ofrece |
|---|---|
| Panel general | Puntuacion de seguridad, vitales en tiempo real, actividad reciente, interruptores de proteccion, analisis completo bajo demanda, aislar equipo |
| Procesos | Listado en vivo ordenado por riesgo, finalizar proceso, bloquear red del proceso |
| Red | Conexiones activas, deteccion de escaneo de puertos, bloquear/desbloquear IP |
| Sistema de ficheros | Eventos de fichero recientes, estado del fichero hosts, deteccion de rafagas |
| Persistencia | Puntos de autoarranque descubiertos, alerta ante entradas nuevas |
| Alertas | Historial completo, reconocer/limpiar alertas |
| Cuarentena | Ficheros aislados con hash SHA-256, restaurar a su ubicacion original |
| Ajustes | Modulos de proteccion, bloqueo automatico, umbrales de deteccion, comprobar actualizaciones |
...
- **Bundle de la web sin code-splitting.** El chunk principal de `website/` supera los 500 KB
  (framer-motion + recharts + iconos); funciona correctamente pero podria dividirse con
  `import()` dinamico para mejorar el tiempo de carga inicial.
